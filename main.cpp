/* 
 * File:   main.cpp
 * Author: Brandon Colindres
 * Created on FEB 10, 2019, 7:36 PM
 * Purpose:  BattleshipV5
 */
...
const char isWATER = '~'; //ASCII Character Code
const char isHIT = 'X';
const char isSHIP = 'B';
const char isMISS = '0';
...
bool UserInputAttack(int& x, int& y, int theplayer)
{
        ...
        bool goodInput = false;
        cin >> x >> y;
        if (x<0 || x>=BWidth) return goodInput;
        if (y<0 || y>=BHeight) return goodInput;
        goodInput = true;
        return goodInput;
}
...
PLACESHIPS UserInputShipPlacement()
{
        ...
        if (x<0 || x>=BWidth) return tmp;
        if (y<0 || y>=BHeight) return tmp;
        //Good data
...
void LoadShips()
{
        ...
        ship[3].name = "carrier"; ship[3].length = 5;
